...
#remove all elements of array that have seqids
...
$progress_bar = Term::ProgressBar->new({    #progress bar for below block
    name    => 'Assessing base composition',
    count   => $max,
    remove  => 1,
});
...
print "Base composition has been assessed for all sequences\n";


#now to determine to position with highest frequency
$max = $#consensus;
$progress_bar = Term::ProgressBar->new({    #progress bar for below block
    name    => 'Compiling consensus',
    count   => $max,
    remove  => 1,
});
